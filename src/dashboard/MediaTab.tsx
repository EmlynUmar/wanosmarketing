...
export const MediaTab: React.FC = () => {
  const { mediaAssets, addMediaAsset, deleteMediaAsset, showToast, saveAgencySettings, agencySettings } = useApp();
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const [newTitle, setNewTitle] = useState('');
  const [newUrl, setNewUrl] = useState('');
  const [newCategory, setNewCategory] = useState<'proof' | 'logo' | 'hero' | 'testimonial'>('proof');
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <form onSubmit={handleAddMedia} className="p-5 rounded-2xl bg-[#0f1622] border border-slate-800 flex flex-col sm:flex-row items-end gap-3 text-xs">
        ...
        <div className="w-full sm:w-36">
          ...
          >
            ...
            <option value="hero">Hero Asset</option>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {mediaAssets.map((asset) => {
          const isCurrentLogo = agencySettings.logoUrl === asset.url;
          return (
            <div key={asset.id} className="p-4 rounded-2xl bg-[#0f1622] border border-slate-800 space-y-3 group hover:border-slate-700 transition-all flex flex-col justify-between">
              <div className="space-y-2">
                <div className="h-44 bg-black rounded-xl overflow-hidden relative border border-slate-800">
                  ...
                  <div className="absolute top-2 left-2 flex items-center gap-1">
                    <span className="px-2 py-0.5 rounded bg-black/80 text-[10px] text-emerald-400 font-semibold border border-white/10">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/80 text-xs">
                ...
                >
                  {isCurrentLogo ? '✓ Current Brand Logo' : 'Set as Logo'}
                </button>
